...
if (isset($_POST['Post'])) {
    ...
    $Profile_idUser = $dbidUser;
    $select = $con->prepare("INSERT INTO message (Profile_idUser, Message, DateTime) VALUES(:Profile_idUser,:Message,now())");
    $select->bindParam(':Profile_idUser', $Profile_idUser);
    $messageStripped = strip_tags($Message);
    $select->bindParam(':Message', $messageStripped);
    $select->execute();
